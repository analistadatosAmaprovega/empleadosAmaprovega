import { useState } from "react";

function generarFechaHoraActual() {
  const ahora = new Date();

  const dia = String(ahora.getDate()).padStart(2, "0");
  const mes = String(ahora.getMonth() + 1).padStart(2, "0");
  const anio = ahora.getFullYear();
  const fecha = `${dia}/${mes}/${anio}`;

  let horas = ahora.getHours();
  const minutos = String(ahora.getMinutes()).padStart(2, "0");
  const segundos = String(ahora.getSeconds()).padStart(2, "0");
  const ampm = horas >= 12 ? "PM" : "AM";
  horas = horas % 12;
  horas = horas === 0 ? 12 : horas;
  const hora = `${String(horas).padStart(2, "0")}:${minutos}:${segundos} ${ampm}`;

  return { fecha, hora };
}

const TIEMPO = {
  ...generarFechaHoraActual(),
  estadoSolicitud: "Expirado",
};

function IconoReloj() {
  return (
    <svg
      className="w-10 h-10 text-white"
      fill="none"
      viewBox="0 0 24 24"
      stroke="currentColor"
      strokeWidth={3}
    >
      <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6l4 2" />
      <circle cx="12" cy="12" r="9" strokeWidth={2} />
    </svg>
  );
}

function Fila({ etiqueta, valor, destacado }) {
  return (
    <div className="flex items-center justify-between py-3">
      <span className="text-sm text-slate-500">{etiqueta}</span>
      <span
        className={
          destacado
            ? "text-sm font-semibold text-orange-600"
            : "text-sm font-semibold text-slate-800"
        }
      >
        {valor}
      </span>
    </div>
  );
}

export default function ReciboQRExpirado({ datos = TIEMPO }) {
  const [visible] = useState(true);

  if (!visible) return null;

  return (
    <div className="w-full min-h-screen bg-slate-900 flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        <div className="flex justify-center -mb-8 relative z-10">
          <div className="w-20 h-20 rounded-full bg-orange-500 flex items-center justify-center shadow-lg ring-8 ring-slate-100">
            <IconoReloj />
          </div>
        </div>

        <div className="bg-white rounded-2xl shadow-md pt-12 pb-6 px-6">
          <div className="text-center mb-4">
            <h1 className="text-xl font-bold text-slate-900">Código QR Expirado</h1>
            <p className="text-sm text-slate-500 mt-1">
              El tiempo de validez de este código ya venció
            </p>
          </div>

          <div className="border-t border-dashed border-slate-300 my-4" />

          <div className="divide-y divide-slate-100">
            <Fila etiqueta="Fecha" valor={datos.fecha} />
            <Fila etiqueta="Hora" valor={datos.hora} />
            <Fila etiqueta="Estado de la solicitud" valor={datos.estadoSolicitud} destacado />
          </div>

          <div className="border-t border-dashed border-slate-300 my-4" />

          <p className="text-center font-bold text-orange-400">
            Escanea el código QR actualizado para continuar
          </p>
        </div>
      </div>
    </div>
  );
}