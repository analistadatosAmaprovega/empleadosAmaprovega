...
// La API devuelve "2026-07-23T04:00:00.000Z" (medianoche local guardada en UTC-4).
// Tomamos solo la parte de fecha para comparar contra el calendario.
function extraerFechaISO(fechaCompleta) {
...
export default function ListadoAsistencia() {
  ...
  const [fechaSeleccionada, setFechaSeleccionada] = useState(formatearFecha(new Date()));
  ...
  useEffect(() => {
    ...
    async function cargarRegistros() {
      ...
      try {
        ...
        if (!resp.ok) {
          throw new Error(`Error ${resp.status} al obtener los registros`);
        }
  ...
  const registrosFiltrados = useMemo(() => {
    if (!fechaSeleccionada) return registros;
    return registros.filter((r) => extraerFechaISO(r.fecha) === fechaSeleccionada);
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                    <th className="text-left font-semibold text-slate-600 px-4 py-3">Empleado</th>
                    ...
                    <th className="text-left font-semibold text-slate-600 px-4 py-3">Ubicación</th>
                  </tr>
                </thead>
                <tbody>
                  {registrosFiltrados.map((registro) => (
                    ...
                    >
                      <td className="px-4 py-3 font-medium">{registro.nombre}</td>
                      <td className="px-4 py-3 text-slate-500">
                        {formatearFechaLegible(extraerFechaISO(registro.fecha))}
                      </td>
                      <td className="px-4 py-3 text-slate-500">{formatearHora(registro.hora)}</td>
                      <td className="px-4 py-3 text-slate-500">{registro.location ?? "--"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
